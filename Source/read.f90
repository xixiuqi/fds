...
NAMELIST /TIME/ DT,DT_END_FILL,DT_END_MINIMUM,DT_EXTERNAL,DT_EXTERNAL_HEARTBEAT,EXTERNAL_HEARTBEAT_FILENAME,HEARTBEAT_FAIL,&
                ...
                RAMP_TIME,RESTRICT_TIME_STEP,T_BEGIN,T_END,TIME_SHRINK_FACTOR,WALL_INCREMENT,T_END_CLOCK
...
CHARACTER(LABEL_LENGTH) :: RAMP_BNDF,RAMP_CPU,RAMP_CTRL,RAMP_DEVC,RAMP_FLUSH,RAMP_GEOM,RAMP_HRR,RAMP_HVAC,RAMP_ISOF,RAMP_MASS,&
                           ...
                           RAMP_TMP,RAMP_SPEC,RAMP_VTK,WRITE_FORMAT
                           !RAMP_SLCF_VTK,RAMP_SL3D_VTK,RAMP_SM3D_VTK,RAMP_BNDF_VTK,RAMP_PART_VTK
NAMELIST /DUMP/ BINGEOM_DIR,CFL_FILE,CLIP_RESTART_FILES,COLUMN_DUMP_LIMIT,CTRL_COLUMN_LIMIT,DEVC_COLUMN_LIMIT,&
                ...
                RAMP_SPEC,RAMP_TMP,RAMP_UVW,RAMP_VTK,RENDER_FILE,RESULTS_DIR,SIG_FIGS,SIG_FIGS_EXP,SMOKE3D,&
                SMV_PARALLEL_WRITE,STATUS_FILES,SUPPRESS_DIAGNOSTICS,TEMP_MAX_SMV,TEMP_MIN_SMV,TURB_INIT_CLOCK,&
                VELOCITY_ERROR_FILE,VTK_BINARY,VTK_DIR,WRITE_CVODE_SUBSTEPS,WRITE_FORMAT,WRITE_XYZ,WRITE_STL
...
SIG_FIGS_EXP       = 3
VTK_BINARY         = .TRUE.
PARAVIEW_PROJECT   = .FALSE.
VTK_DIR            = ''
WRITE_VTK_GEOM     = .FALSE.
WRITE_VTK          = .FALSE.
WRITE_SMV          = .TRUE.
WRITE_FORMAT       = 'SMV'
IF (NMESHES>32) THEN
...
IF (DT_VTK_SPECIFIED    /= DT_VTK)     THEN ; DT_VTK_SPECIFIED    = DT_VTK     ; ELSE ; DT_VTK_SPECIFIED   = -1._EB ; ENDIF

IF (DT_VTK_SPECIFIED > 0) THEN ; WRITE_VTK=.TRUE.; ENDIF
IF (WRITE_VTK) THEN
   PARAVIEW_PROJECT=.TRUE.
   WRITE_VTK_GEOM=.TRUE.
   IF (DT_VTK_SPECIFIED > 0) THEN
      DT_SLCF_VTK = DT_VTK_SPECIFIED
      DT_SL3D_VTK = DT_VTK_SPECIFIED
      DT_SM3D_VTK = DT_VTK_SPECIFIED
      DT_BNDF_VTK = DT_VTK_SPECIFIED
      DT_PART_VTK = DT_VTK_SPECIFIED
   ELSE
      IF (DT_SLCF_SPECIFIED > 0) THEN ; DT_SLCF_VTK = DT_SLCF_SPECIFIED ; ELSE ; DT_SLCF_VTK = -1._EB ; ENDIF
      IF (DT_SL3D_SPECIFIED > 0) THEN ; DT_SL3D_VTK = DT_SL3D_SPECIFIED ; ELSE ; DT_SL3D_VTK = -1._EB ; ENDIF
      IF (DT_SMOKE3D_SPECIFIED > 0) THEN ; DT_SM3D_VTK = DT_SMOKE3D_SPECIFIED ; ELSE ; DT_SM3D_VTK = -1._EB ; ENDIF
      IF (DT_BNDF_SPECIFIED > 0) THEN ; DT_BNDF_VTK = DT_BNDF_SPECIFIED ; ELSE ; DT_BNDF_VTK = -1._EB ; ENDIF
      IF (DT_PART_SPECIFIED > 0) THEN ; DT_PART_VTK = DT_PART_SPECIFIED ; ELSE ; DT_PART_VTK = -1._EB ; ENDIF
   ENDIF
ENDIF
...
!IF (RAMP_SLCF_VTK    /='null') CALL GET_RAMP_INDEX(RAMP_SLCF_VTK    ,'TIME',RAMP_SLCF_VTK_INDEX)
!IF (RAMP_SL3D_VTK    /='null') CALL GET_RAMP_INDEX(RAMP_SL3D_VTK    ,'TIME',RAMP_SL3D_VTK_INDEX)
!IF (RAMP_SM3D_VTK    /='null') CALL GET_RAMP_INDEX(RAMP_SM3D_VTK    ,'TIME',RAMP_SM3D_VTK_INDEX)
!IF (RAMP_BNDF_VTK    /='null') CALL GET_RAMP_INDEX(RAMP_BNDF_VTK    ,'TIME',RAMP_BNDF_VTK_INDEX)
!IF (RAMP_PART_VTK    /='null') CALL GET_RAMP_INDEX(RAMP_PART_VTK    ,'TIME',RAMP_PART_VTK_INDEX)
...
REAL(EB) :: DIAMETER, GAMMA_D,AGE,INITIAL_TEMPERATURE,HEAT_OF_COMBUSTION, KILL_MASS_FRACTION, &
...
NAMELIST /PART/ ADHERE_TO_SOLID,AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                CTRL_ID,DEBUG,DENSE_VOLUME_FRACTION,KILL_MASS_FRACTION,&
...
READ_PART_LOOP: DO N=1,N_LAGRANGIAN_CLASSES
   ...
   LPC%KILL_MASS_FRACTION = KILL_MASS_FRACTION
...
SELECT CASE(TRIM(SOLVER))
   ...
   CASE('UGLMAT','UGLMAT PARDISO','UGLMAT HYPRE')
      ...
      IF (MAX_PRESSURE_ITERATIONS==10) MAX_PRESSURE_ITERATIONS=1
...
REAL(EB), ALLOCATABLE, DIMENSION(:) :: AGL_SLICES
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   ! Count unique slices and allocate memory
   IF (.NOT.ALLOCATED(UNIQUE_SLICE_NAMES)) THEN
      ALLOCATE(UNIQUE_SLICE_NAMES(N_SLCF_O*4))
   ENDIF
   ALLOCATE(AGL_SLICES(N_SLCF_O*4))
   CALL COUNT_UNIQUE_SLCF(N_SLCF_O,N_UNIQUE_SLCF,UNIQUE_SLICE_NAMES,AGL_SLICES)
   M%N_UNIQUE_SLCF=N_UNIQUE_SLCF
   MESHES(1)%N_UNIQUE_SLCF = N_UNIQUE_SLCF

   ALLOCATE(M%UNIQUE_SLICE_NAMES(N_UNIQUE_SLCF))
   ALLOCATE(M%EMPTY_UNIQUE_SLICE(N_UNIQUE_SLCF))
   ALLOCATE(M%UNIQUE_SLICE_IS_SL3D(N_UNIQUE_SLCF))

   IF (.NOT.ALLOCATED(MESHES(1)%ALL_SLICE_NAMES)) THEN
      ALLOCATE(MESHES(1)%ALL_SLICE_NAMES(N_SLCF_O*4))
      ALLOCATE(MESHES(1)%ALL_SLICE_QUANTITIES(N_SLCF_O*4))
      ALLOCATE(MESHES(1)%ALL_SLICE_TOPOLOGIES(N_SLCF_O*4))
      DO NN=1,N_SLCF_O*4
         MESHES(1)%ALL_SLICE_NAMES(NN)=""
         MESHES(1)%ALL_SLICE_QUANTITIES(NN)=""
         MESHES(1)%ALL_SLICE_TOPOLOGIES(NN)=-1
      ENDDO
   ...
   ! Initialize allocated mesh variables
   M%UNIQUE_SLICE_NAMES = UNIQUE_SLICE_NAMES(1:N_UNIQUE_SLCF)
   DO I = 1,N_UNIQUE_SLCF
      M%EMPTY_UNIQUE_SLICE(I)=.TRUE.
   ENDDO
   ...
   SLCF_LOOP: DO NN=1,N_SLCF_O
      ...
      IF (AGL_SLICE>=0._EB .AND. CELL_CENTERED) THEN
         ...
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF ((QUANTITY=='TIME OF ARRIVAL' .AND. AGL_SLICE<0) .OR. &
         (QUANTITY=='RESIDENCE TIME' .AND. AGL_SLICE<0))THEN
         WRITE(MESSAGE,'(A)') 'ERROR(962): FIRE ARRIVAL TIME or RESIDENCE TIME only permitted with AGL_SLICE.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB) THEN
         XB(1) = XS
      ...
      VECTORLOOPNAME: DO ITER=1,NITER
         N1 = N1 + 1
         MESHES(1)%ALL_SLICE_NAMES(N1) = TRIM(SLCF_NAME)
         !WRITE(*,*) TRIM(SLCF_NAME), " ", PBX, " ", PBY, " ", PBZ, " ", PBX>-1.E5_EB, " ", PBY>-1.E5_EB, " ", PBZ>-1.E5_EB
         IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB) THEN
            MESHES(1)%ALL_SLICE_TOPOLOGIES(N1) = 0
         ELSEIF (AGL_SLICE > 0) THEN
            MESHES(1)%ALL_SLICE_TOPOLOGIES(N1) = 0
         ELSE
            MESHES(1)%ALL_SLICE_TOPOLOGIES(N1) = 1
         ENDIF
         !WRITE(*,*) TRIM(SLCF_NAME), MESHES(1)%ALL_SLICE_TOPOLOGIES(N1)
         QUANTITY_TMP = QUANTITY
         IF (ITER==2)                    QUANTITY_TMP = 'U-VELOCITY'
         IF (ITER==3 .AND. .NOT. TWO_D)  QUANTITY_TMP = 'V-VELOCITY'
         IF (ITER==3 .AND. TWO_D)        QUANTITY_TMP = 'W-VELOCITY'
         IF (ITER==4)                    QUANTITY_TMP = 'W-VELOCITY'
         CALL GET_QUANTITY_INDEX(SMOKEVIEW_LABEL,SMOKEVIEW_BAR_LABEL,INDEX,INDEX2, &
            Y_INDEX,Z_INDEX,I_DUM(1),PART_INDEX,I_DUM(2),I_DUM(3),REAC_INDEX,MATL_INDEX,'SLCF', &
            QUANTITY_TMP,QUANTITY2,SPEC_ID,'null',PART_ID,'null','null',REAC_ID,'null',-1._EB,I_DUM(4),&
            SLICETYPE=SLICETYPE)
         MESHES(1)%ALL_SLICE_QUANTITIES(N1) = SMOKEVIEW_LABEL
      ENDDO VECTORLOOPNAME
      MESHES(1)%N_SLCF_VTK = N1

      DO I = 1,N_UNIQUE_SLCF
         IF (TRIM(SLCF_NAME).EQ.TRIM(UNIQUE_SLICE_NAMES(I))) THEN
            IF (MESHES(1)%ALL_SLICE_TOPOLOGIES(N1).EQ.1) THEN
               M%UNIQUE_SLICE_IS_SL3D(I)=.TRUE.
            ELSE
               M%UNIQUE_SLICE_IS_SL3D(I)=.FALSE.
            ENDIF
   ...
   DO NN = 1,N_SLCF
      ...
      ENDIF
      DO I = 1,N_UNIQUE_SLCF
         IF (SL%SLCF_NAME.EQ.UNIQUE_SLICE_NAMES(I)) THEN
            M%EMPTY_UNIQUE_SLICE(I)=.FALSE.
         ENDIF
      ENDDO
   ENDDO
   IF (.NOT.ALLOCATED(MESHES(1)%UNIQUE_SLICE_NAMES)) THEN
      ALLOCATE(MESHES(1)%UNIQUE_SLICE_NAMES(N_UNIQUE_SLCF))
      MESHES(1)%UNIQUE_SLICE_NAMES = M%UNIQUE_SLICE_NAMES
   ENDIF
...
IF (PBX>-1.E5_EB .OR. PBY>-1.E5_EB .OR. PBZ>-1.E5_EB) THEN
   ...
   ENDIF
ELSEIF (AGL_SLICE > 0) THEN
   WRITE(SLCF_NAME,'(A,I6.6,I2.2)') 'AGL_',FLOOR(AGL_SLICE),FLOOR((AGL_SLICE-FLOOR(AGL_SLICE))*100)
ELSE
   XS = ABS(XS_MIN)
   XF = ABS(XF_MAX)
   YS = ABS(YS_MIN)
...
INTEGER :: N_UNIQUE_SLCF,N,I
CHARACTER(200) :: SLCF_NAME
CHARACTER(200), DIMENSION(N_SLCF_O*3) :: UNIQUE_SLICE_NAMES
REAL(EB), DIMENSION(N_SLCF_O*3) :: AGL_SLICES
LOGICAL :: UNIQUE_SLICE

N_UNIQUE_SLCF=0
DO N = 1,N_SLCF_O*3
   UNIQUE_SLICE_NAMES(N) = ""
ENDDO

REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0

SLCF_LOOP: DO N=1,N_SLCF_O
   PBX      = -1.E6_EB
   PBY      = -1.E6_EB
   PBZ      = -1.E6_EB
   DB       = 'null'
   ...
   UNIQUE_SLICE = .TRUE.
   DO I = 1,N_UNIQUE_SLCF+1
      IF (SLCF_NAME.EQ.UNIQUE_SLICE_NAMES(I)) THEN
         UNIQUE_SLICE = .FALSE.
      ENDIF
   ENDDO
   IF (UNIQUE_SLICE) THEN
      N_UNIQUE_SLCF = N_UNIQUE_SLCF + 1
      UNIQUE_SLICE_NAMES(N_UNIQUE_SLCF) = SLCF_NAME
      AGL_SLICES(N_UNIQUE_SLCF) = AGL_SLICE
   ENDIF
ENDDO SLCF_LOOP

END SUBROUTINE COUNT_UNIQUE_SLCF
...
READ_BNDF_LOOP: DO N=1,N_BNDF
   ...
   IF (TRIM(QUANTITY)=='AMPUA_Z' .OR. TRIM(QUANTITY)=='CPUA_Z' .OR. TRIM(QUANTITY)=='MPUA_Z') THEN
      IF (N_LP_ARRAY_INDICES == 0) THEN
         WRITE(MESSAGE,'(A,I0,A)') 'ERROR(964): BNDF ',N,' CPUA_Z, MPUA_Z, and AMPUA_Z require liquid droplets.'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ELSE
